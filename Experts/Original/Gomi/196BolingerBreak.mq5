...
input int BandPeriod,ATRPeriod;
...
int OnInit() {
  ...
  myutils.Init();
  Bands.Create(_Symbol,Timeframe,BandPeriod,0,2,AppliedPrice);
...
void OnTick() {
  ...
  if(!myTrade.istradable || !tradable) return;
	...
	if(Bands.Upper(2) < myPrice.At(2).high && Bands.Upper(1) < myPrice.At(1).close) myTrade.setSignal(ORDER_TYPE_BUY);
	if(Bands.Lower(2) < myPrice.At(2).low && Bands.Lower(1) > myPrice.At(1).close) myTrade.setSignal(ORDER_TYPE_SELL);


  double PriceUnit = ATR.Main(0);
  if(myPosition.TotalEachPositions(POSITION_TYPE_BUY) < positions / 2 ) {
    double SL = Bands.Lower(0);
    double TP = myTrade.Ask + PriceUnit*TPCoef;
    myTrade.Buy(SL,TP);
  }
  if(myPosition.TotalEachPositions(POSITION_TYPE_SELL) < positions / 2 ) {
    double SL = Bands.Upper(0);
