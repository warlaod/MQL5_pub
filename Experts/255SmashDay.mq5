...
int OnInit() {
   ...
   myutils.Init();
   ATR.Create(_Symbol, Timeframe, 5);
...
void OnTick() {
   ...
   myPosition.Trailings(POSITION_TYPE_BUY, myTrade.Ask - ATR.Main(0), myTrade.Ask + 10 * pips);
   myPosition.Trailings(POSITION_TYPE_SELL, myTrade.Bid + ATR.Main(0), myTrade.Bid - 10 * pips);
   ...
   if(myPrice.RosokuIsPlus(2) && myPrice.RosokuIsPlus(1)) {
      if(myPrice.At(2).high < myPrice.At(1).close)
         if(myPrice.At(1).low > myPrice.At(0).close)
            myTrade.setSignal(ORDER_TYPE_SELL);
   }

   if(!myPrice.RosokuIsPlus(2) && !myPrice.RosokuIsPlus(1)) {
      if(myPrice.At(2).low > myPrice.At(1).close)
   ...
   if(myPosition.TotalEachPositions(POSITION_TYPE_BUY) < positions) {
      myTrade.Buy(myPrice.At(1).low, myTrade.Ask + PriceUnit * TPCoef);
   }
   if(myPosition.TotalEachPositions(POSITION_TYPE_SELL) < positions) {
      myTrade.Sell(myPrice.At(1).high, myTrade.Bid - PriceUnit * TPCoef);
   }
