...
int Indicator;
double UpFractalLast,LowFractalLast;
...
double Bull[],Bear[];
int BullIndicator,BearIndicator;
int BullBearPeriod,BullBearParam;
input double BullLongCri,BullShortCri,BearLongCri,BearShortCri;
...
int OnInit()
  {
   ...
   BullIndicator = iBullsPower(_Symbol,Timeframe(BullBearPeriod),BullBearParam);
   BearIndicator = iBearsPower(_Symbol,Timeframe(BullBearPeriod),BullBearParam);
...
//--- finding the second upper fractal 
   ...
   LowFractal_old=FractalUp[n];




   CopyBuffer(OsmaIndicator,0,0,2,Osma);
   CopyBuffer(BullIndicator,0,0,2,Bull);
   CopyBuffer(BearIndicator,0,0,2,Bear);

   signal = "";

   if(Osma[0] > 0 && Osma[0] > Osma[1] && Osma[0] > OsmaCriPL && Bull[0] > BullLongCri && Bear[0] > BearLongCri)
     {
      if(UpFractal != UpFractalLast)
        {
         UpFractalLast = UpFractal;
         signal= "buy";
        }
     }
   else
      if(Osma[0] < 0 && Osma[0] < Osma[1] && Osma[0] < -OsmaCriMI && Bull[0] < BullShortCri && Bear[0] < BearShortCri)
        {
         if(LowFractal != LowFractalLast)
           {
            LowFractalLast = LowFractal;
            signal= "sell";
           }
        }
