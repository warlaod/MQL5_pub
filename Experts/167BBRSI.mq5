...
MyTrade myTrade(0.1);
MyPrice myPrice(RSITimeframe,2);
...
int OnInit() {
   ...
   //ciATR.Create(_Symbol, RSITimeframe, ATRPeriod);
...
void OnTick() {
   ...
   double Highest = myPrice.Higest();
   double Lowest = myPrice.Lowest();
   double PriceUnit = ciATR.Main(0);
   if(myPosition.TotalEachPositions(POSITION_TYPE_BUY) < positions / 2 && myTrade.signal == "buy") {
      if(myTrade.isInvalidTrade(myPrice.Lowest(),ciBands.Base(0))) return;
      trade.Buy(myTrade.lot, NULL, myTrade.Ask, myPrice.Lowest(), ciBands.Base(0), NULL);
   }

   if(myPosition.TotalEachPositions(POSITION_TYPE_SELL) < positions / 2 && myTrade.signal == "sell") {
      if(myTrade.isInvalidTrade(myPrice.Higest(), ciBands.Base(0))) return;
      trade.Sell(myTrade.lot, NULL, myTrade.Bid, myPrice.Higest(),ciBands.Base(0), NULL);
   }
}
//+------------------------------------------------------------------+
//|                                                                  |
